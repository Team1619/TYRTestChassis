...
//daniel senpai said to add another comment
  ...
  public Robot() {
    ...
    drive = new RobotDrive(leftDriveMotor1, leftDriveMotor2, rightDriveMotor1,
      rightDriveMotor2);
  ...
  private void displayVals() {
    if (imageReceiveValue > imageReceivePeriod) {
      ...
      if (contourTableVals.length > 0) {
        ...
        for (int i = 0; i < contourTableVals.length; i++) {
          SmartDashboard.putString("Contour" + i,
            Arrays.toString(contourTableVals[i]));
        }
      }
      ...
      if (lineTableVals.length > 0) {
        ...
        for (int i = 0; i < lineTableVals.length; i++) {
          SmartDashboard.putString("Line" + i,
            Arrays.toString(lineTableVals[i]));
        }
      }
    ...
    SmartDashboard.putNumber("Right Encoder",
      rightDriveMotor1.getEncPosition());
  ...
  private void makeQuadrilateral() {
    ...
    if (perimeter.length > 0 && contourStats.length > 0) {
      ...
      SmartDashboard.putString("leftBottomCoords",
        Arrays.toString(leftBottomCoords));
      SmartDashboard.putString("rightTopCoords",
        Arrays.toString(rightTopCoords));
      SmartDashboard.putString("rightBottomCoords",
        Arrays.toString(rightBottomCoords));

      double slopeUpper = Math
        .toDegrees(Math.atan((rightTop - leftTop) / (rightTopX - leftTopX)));
      double slopeLower = Math.toDegrees(
        Math.atan((rightBottom - leftBottom) / (rightBottomX - leftBottomX)));
      double slopeLeft = Math.toDegrees(
        Math.atan((leftBottom - leftTop) / (leftTopX - leftBottomX)));
      double slopeRight = Math.toDegrees(
        Math.atan((rightBottom - rightTop) / (rightTopX - rightBottomX)));
      ...
      double[] topMidpoint =
        {(leftTopX + rightTopX) / 2, (leftTop + rightTop) / 2};
      double[] bottomMidpoint =
        {(leftBottomX + rightBottomX) / 2, (leftBottom + rightBottom) / 2};
      // SmartDashboard.putString("Top midpoint", Arrays.toString(topMidpoint));
      // SmartDashboard.putString("Bottom midpoint",
      // Arrays.toString(bottomMidpoint));
    ...
    String[] dataProblems = {"Contour analyzed", "Missing lines",
      "Missing contour stats", "Missing both"};
